...
const mysql_util = require('./mysql/mysql_util')
...
ZCRMRestClient.generateAuthTokens = function(user_identifier, grant_token) {
  return new Promise(function(resolve, reject) {
    ...
    OAuth.generateTokens(api_url).then(function(response) {
      ...
      const mysql_util = {...ZCRMRestClient.getMySQLModule()};
...
ZCRMRestClient.parseAndConstructObject = function(response) {
  ...
  if (body.access_token) {
    ...
    const expires_in_MS = body.expires_in * 1000; // turn 3600s into milliseconds

    resultObj.expires_in = expires_in_MS + current_time - oneMinuteInMs;
  }
  return resultObj;
};
